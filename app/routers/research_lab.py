...
from app.services.personal_calibration_validation_service import PersonalCalibrationValidationService

router = APIRouter()
_lab = ResearchLabService(ROOT)
_evaluator = ModelEvaluationService(ROOT)
_calibration_validator = PersonalCalibrationValidationService(ROOT)
...
@router.get("/calibration-validation")
def calibration_validation_status():
    if not _lab.deployment_mode()["research_enabled"]:
        raise HTTPException(
            status_code=403,
            detail="Calibration Validation is disabled in this deployment.",
        )
    return _calibration_validator.snapshot()


@router.post("/calibration-validation")
def run_calibration_validation(payload: CalibrationValidationRequest):
    if not _lab.deployment_mode()["research_enabled"]:
        raise HTTPException(
            status_code=403,
            detail="Calibration Validation is disabled in this deployment.",
    ...
    try:
        return _calibration_validator.validate(
            model_path=payload.model_path,
            test_path=payload.test_path,
        )
    except FileNotFoundError as error:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Calibration validation failed: {type(error).__name__}: {error}",
        ) from error


@router.get("/calibration-validation/export")
def export_calibration_validation():
    if not _lab.deployment_mode()["research_enabled"]:
        raise HTTPException(
            status_code=403,
            detail="Calibration Validation export is disabled in this deployment.",
        )
    path = ROOT / "guardian_data" / "research_lab" / "latest_calibration_validation.json"
    if not path.exists():
        raise HTTPException(
            status_code=404,
            detail="Run calibration validation before exporting.",
        )
    return FileResponse(
        path,
        media_type="application/json",
