...
function openPrivacyNotice() {
  const modal = el("privacy-modal");
  const checkbox = el("privacy-understood");
  const continueButton = el("privacy-continue");
...
el("privacy-continue")?.addEventListener("click", async () => {
  if (!el("privacy-understood").checked) return;
...
el("privacy-cancel")?.addEventListener("click", () => {
  closePrivacyNotice();
});

el("monitor-start")?.addEventListener("click", () => {
  if (DG.state?.metrics?.monitoring || DG.state?.metrics?.starting) return;
  openPrivacyNotice();
});
...
if (el("monitor-mute-alerts")) el("monitor-mute-alerts").textContent = DG.alertsMuted ? "Unmute alerts" : "Mute alerts";
