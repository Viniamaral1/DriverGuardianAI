...
class CommanderService:
    ...
    def answer(self, message: str) -> str:
        ...
        profiles = (
            self.state.driver_profile_service.snapshot()
            if self.state.driver_profile_service is not None
            else {"active_profile": None, "guest_mode": True}
        )
        try:
        ...
        if any(term in q for term in (
            ...
            if not active:
                return (
                    "Guest mode is active. Guardian will use the original full "
                    "calibration and will not save or reuse a personal baseline."
                )
            calibration = active.get("calibration")
            if calibration:
                return (
                    f"The active driver profile is {active.get('name')}. "
                    "A saved baseline is available, so the next session begins "
                    "with a three-second verification and automatically falls "
                    "back to the full calibration if the measurements do not match."
                )
            return (
                f"The active driver profile is {active.get('name')}. "
                "No saved baseline exists yet, so the next session will use the "
                "full calibration and save the result locally."
            )
        ...
        if any(term in q for term in (
            ...
            return (
                f"Calibration status is {metrics.get('calibration_status', 'full calibration')}. "
                f"Active profile is {metrics.get('driver_profile_name', 'Guest')}. "
                f"Remaining calibration time is "
                f"{float(metrics.get('calibration_remaining', 0) or 0):.1f} seconds."
            )
