...
function renderCalibration(metrics) {
  ...
  const required = Number(metrics.calibration_required_seconds || 10);
  const progress = Math.max(
    0,
    Math.min(1, (required - remaining) / Math.max(required, 0.1))
  );
  const mode = String(metrics.calibration_mode || "full");
...
function renderState(metrics) {
  ...
  byId("driver-profile-badge").textContent =
    String(metrics.driver_profile_name || "Guest").toUpperCase();
