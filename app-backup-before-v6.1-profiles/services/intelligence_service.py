...
class IntelligenceService:
    ...
    def _signal_quality(self, metrics: dict[str, Any]) -> dict[str, Any]:
        ...
        score = 0.0
        score += 0.45 if face else 0.0
        score += 0.30 if calibrated else 0.0
        score += min(0.25, max(0.0, fps) / 80.0)
        ...
        if not face:
            ...
            level = "moderate"
            summary = "Signals are available, but the camera frame rate is low."
        else:
            level = "good"
            summary = "Face, calibration and frame-rate signals are available."
